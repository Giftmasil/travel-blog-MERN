...
export default function Settings() {
  ...
  useEffect(() => {
    const fetchUserPosts = async () => {
      try {
        const res = await axios.get(`https://travel-blog-mern-1.onrender.com/api/posts?user=${user.username}`);
    ...
    const fetchNotifications = async () => {
      try {
        const res = await axios.get(`https://travel-blog-mern-1.onrender.com/api/notifications/${user._id}`);
    ...
    const fetchFollowsAndFollowings = async () => {
      try {
        const followsRes = await axios.get(`https://travel-blog-mern-1.onrender.com/api/users/${user._id}/followers`);
        const followingsRes = await axios.get(`https://travel-blog-mern-1.onrender.com/api/users/${user._id}/followings`);
  ...
  const handleMarkAsRead = async (id) => {
    try {
      await axios.put(`https://travel-blog-mern-1.onrender.com/api/notifications/${id}/read`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (file) {
      ...
      try {
        await axios.post("https://travel-blog-mern-1.onrender.com/api/upload", data);
      } catch (err) {
        console.error("Error uploading file:", err);
      }
    }
    try {
      const res = await axios.put("https://travel-blog-mern-1.onrender.com/api/users/" + user._id, updatedUser);
      setSuccess(true);
